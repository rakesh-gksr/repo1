...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvPipingDropdownSinglePageMode/",  # report_relative_location
                               "test_advPiping_dropdown_singlePageMode",  # report_file_name_prefix
                               "Verify advance piping from Dropdown qtype to all qtype except NPS",  # test_suite_title
                               ("This test adds Dropdown type question "
...
def test_advPiping_dropdown_singlePageMode_multiChoice(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to all qtype except NPS.")
    ...
    try:
        ...
        mySurvey.myDesign.enter_dropdown_preview(1, "Yuudachi")
...
def test_advPiping_dropdown_singlePageMode_dropdown(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to dropdown type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_matrix(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to matrix type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_menuMatrix(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to menu matrix type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_ranking(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to ranking type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_singleTextbox(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to Single Textbox type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_multiTextbox(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to Multi Textbox type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_commentBox(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to Comment Box type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_contactInfo(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to Contact Info type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_dateTime(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to DateTime type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_text(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to Text type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_image(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to Image type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_textAB(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to TextAB  type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
...
def test_advPiping_dropdown_singlePageMode_imageAB(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Dropdown qtype to ImageAB type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_dropdown_question(
