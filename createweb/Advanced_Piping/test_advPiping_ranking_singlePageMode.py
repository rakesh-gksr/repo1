...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvPipingRankingSinglePageMode/",  # report_relative_location
                               "test_advPiping_ranking_singlePageMode",  # report_file_name_prefix
                               "Verify advance piping from Ranking qtype to all qtype except NPS",  # test_suite_title
                               ("This test adds Ranking type question "
...
def test_advPiping_ranking_singlePageMode_multiChoice(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to all qtype except NPS.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
        ...
        mySurvey.myLogic.process_dropdown_preview(1, 1, 1)
...
def test_advPiping_ranking_singlePageMode_dropdown(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to dropdown type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_matrix(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to matrix type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_menuMatrix(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to menu matrix type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_ranking(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to ranking type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_singleTextbox(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to Single Textbox type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_multiTextbox(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to Multi Textbox type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_commentBox(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to Comment Box type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_contactInfo(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to Contact Info type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_dateTime(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to DateTime type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_text(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to Text type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_image(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to Image type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_textAB(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to TextAB  type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
...
def test_advPiping_ranking_singlePageMode_imageAB(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify advanced piping from Ranking qtype to ImageAB type question.")
    new_survey_id = init_single_page_mode_survey(driver, mySurvey, report)
    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myQuestion.generate_ranking_question(new_survey_id, page_num, "Best Vocaloid?", 1)
