...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchConditionPageMoveDown/",  # report_relative_location
                               "test_advBranch_conditionPageMoveDown",  # report_file_name_prefix
                               "Test reordering pages after adding rules- reorder conditional page below destination page",  # test_suite_title
...
def test_advBranch_conditionMoveDown(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(
        ReportMessageTypes.TEST_CASE,
        "Test reordering pages after adding rules- reorder conditional page below destination page.")
    try:
        ...
        mySurvey.myLogic.movePage(1, 3)
        ex = mySurvey.myLogic.verify_num_advanced_branching(1, ["SKIP TO", "P2"])
        ex = not ex  # with new changes, potential infinite looping causes advanced branching to turn off.
