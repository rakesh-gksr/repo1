...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchEditExisting/",  # report_relative_location
                               "test_advBranch_editExisting",  # report_file_name_prefix
                               "verify editing existing survey maintains advanced branching",  # test_suite_title
...
@pytest.mark.MT1
def test_advBranch_editExisting(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify editing existing survey maintains advanced branching")
    try:
