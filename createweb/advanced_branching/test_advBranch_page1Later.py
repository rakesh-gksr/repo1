...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchPage1Later/",  # report_relative_location
                               "test_advBranch_page1Later",  # report_file_name_prefix
                               "Test branching from the second page to a later page based on questions on the first page",  # test_suite_title
...
def test_advBranch_page1Later(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(
        ReportMessageTypes.TEST_CASE,
        "Test branching from the second page to a later page based on questions on the first page.")
    try:
        ...
        ex = mySurvey.myLogic.addNewBranchingRule([
            ("MultipleChoice", [1, 1], "response", [None], "skip", "default", 4)])
        ...
        ex = mySurvey.myLogic.verify_advanced_branching("multiChoice", 1, [1], "future_skip", [4, "Page 4"])
