...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchQuestionDelete/",  # report_relative_location
                               "test_advBranch_questionDelete",  # report_file_name_prefix
                               "Test deleting questions after adding rules",  # test_suite_title
...
def test_advBranch_questionDelete(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test deleting questions after adding rules.")
    try:
