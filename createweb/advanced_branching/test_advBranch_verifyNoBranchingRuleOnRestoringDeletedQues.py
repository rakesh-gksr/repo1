...
# -*- coding: utf-8 -*-
"""
Verifies that the advance branching logic is removed on restoring the deleted question on single page.
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchVerifyNoBranchRuleOnRestoringDelQues/",  # report_relative_location
                               "test_advBranch_verifyNoBranchingRuleOnRestoringDeletedQues",  # report_file_name_prefix
                               "To check that Branching rule does not exists after restoring a deleted Matrix type"
                               " question on single page survey",  # test_suite_title
                               ("verify deleting source question and restoring it back shouldn't restore branching rule"
                                "with one page survey"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.IB
@pytest.mark.C249901
def test_advBranch_verifyNoBranchingRuleOnRestoringDeletedQues(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify deleting source question and restoring it back "
                                                           "shouldn't restore branching rule with one page survey")

    try:
        page_num = mySurvey.myDesign.getPageID(1)
        mySurvey.myDesign.click_addPageTitle("Page 1", "This is test page 1")
        mySurvey.myQuestion.generate_unweighted_matrix_question(mySurvey.survey_id,
                                                                page_num, "Please classify the following Ships",
