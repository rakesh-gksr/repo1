...
from dynamic_tests.dynamic_tests_predicates_config import dropdown_qtype_predicates, survey_dict
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict["dropdown_qtype"]
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchVerifyQTypeDropdownPredicates/",
                               # report_relative_location
                               "test_advBranch_verifyQTypeDropdownPredicates",
                               # report_file_name_prefix
                               ("Verify setting condition selecting all the predicates in condition menu and "
                                "select diff actions for each predicate for dropdown question type"),
                               # test_suite_title
...
@pytest.mark.IB
@pytest.mark.C281047
@pytest.mark.parametrize("test_info", dropdown_qtype_predicates,
                         ids=[dict["predicate_label"] for dict in dropdown_qtype_predicates])
def test_advBranch_verifyQTypeDropdownPredicates(create_survey, test_info):
    """
     Description :
