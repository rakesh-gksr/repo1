...
# -*- coding: utf-8 -*-
"""
1. This test case adds different predicates in condition menu and select diff actions for each predicate,
2. It also verify that they works as expected in Preview & Test.
"""
from smsdk.qafw.create.create_utils import reporting_wrapper, env_init
from smsdk.qafw.reporting.report_message_types import ReportMessageTypes
from smsdk.qafw.reporting.splunk_data_attributes import SplunkDataAttributes
from smsdk.qafw.reporting.splunk_test_case_types import SplunkTestCaseTypes
from dynamic_tests.dynamic_tests_predicates_config import matrix_of_dropdown_menus_predicates, survey_dict
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict["matrix_menus_qtype"]
    new_survey_id = mySurvey.myCreate.copy_exiting_survey(survey_info["survey_id"],
                                                          survey_info["user_id"],
                                                          survey_title +
                                                          " Copied via svysvc")
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchVerifyQTypeMatrixOfDropdownMenuPredicates/",
                               # report_relative_location
                               "test_advBranch_verifyQTypeMatrixOfDropdownMenuPredicates",
                               # report_file_name_prefix
                               ("Verify setting condition selecting all the predicates in condition menu and select "
                                "diff actions for each predicate for Matrix/Rating Scale question type"),
                               # test_suite_title
                               ("This test suite adds different predicates in condition menu and select "
                                "diff actions for each predicate and verify "
...
@pytest.mark.IB
@pytest.mark.C281052
@pytest.mark.parametrize("test_info", matrix_of_dropdown_menus_predicates,
                         ids=[dict["predicate_label"] for dict in matrix_of_dropdown_menus_predicates])
def test_advBranch_verifyQTypeMatrixOfDropdownMenuPredicates(create_survey, test_info):
    """
     Description :
    ...
    try:
        ...
        # Below code copy existing survey and loaded survey into browser
