...
from dynamic_tests.dynamic_tests_predicates_config import (multiple_choice_multiple_answer_predicates,
                                                           survey_dict)
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict["multiple_choice_multi_answer"]
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranch_verifyQTypeMultipleChoiceMultipleAnswerPredicates/",
                               # report_relative_location
                               "test_advBranch_verifyQTypeMultipleChoiceMultipleAnswerPredicates",
                               # report_file_name_prefix
                               ("Verify setting condition selecting all the predicates in condition menu and select "
                                "diff actions for each predicate for multiple choice multiple answer question type"),
                               # test_suite_title
...
@pytest.mark.IB
@pytest.mark.C281008
@pytest.mark.parametrize("test_info", multiple_choice_multiple_answer_predicates,
                         ids=[dict["predicate_label"] for dict in multiple_choice_multiple_answer_predicates])
def test_advBranch_verifyQTypeMultipleChoiceMultipleAnswerPredicates(create_survey, test_info):
    """
     Description :
