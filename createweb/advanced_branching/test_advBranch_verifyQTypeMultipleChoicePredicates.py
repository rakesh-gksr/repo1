...
from dynamic_tests.dynamic_tests_predicates_config import multiple_choice_predicates, survey_dict
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict["multiple_choice"]
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranch_verifyQTypeMultipleChoicePredicates/",
                               # report_relative_location
                               "test_advBranch_verifyQTypeMultipleChoicePredicates",
                               # report_file_name_prefix
                               ("Verify setting condition selecting all the predicates in condition menu and select"
                                " diff actions for each predicate for multiple choice question type"),
                               # test_suite_title
...
@pytest.mark.IB
@pytest.mark.C278342
@pytest.mark.parametrize("test_info", multiple_choice_predicates,
                         ids=[dict["predicate_label"] for dict in multiple_choice_predicates])
def test_advBranch_verifyQTypeMultipleChoicePredicates(create_survey, test_info):
    """
     Description :
