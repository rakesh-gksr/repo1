...
from dynamic_tests.dynamic_tests_predicates_config import multiple_textbox_numeric_predicates, survey_dict
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict["numeric_multiple_textbox"]
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchVerifyQTypeMultipleTextboxNumericPredicates/",
                               # report_relative_location
                               "test_advBranch_verifyQTypeMultipleTextboxNumericPredicates",
                               # report_file_name_prefix
                               "verify setting condition selecting all the predicates in condition menu and select"
                               " diff actions for each predicate for Multiple Textboxes with numerical data question"
                               " type",
                               # test_suite_title
...
@pytest.mark.IB
@pytest.mark.C281057
@pytest.mark.parametrize("test_info", multiple_textbox_numeric_predicates,
                         ids=[dict["predicate_label"] for dict in multiple_textbox_numeric_predicates])
def test_advBranch_verifyQTypeMultipleTextboxNumericPredicates(create_survey, test_info):
    """
    Description :
