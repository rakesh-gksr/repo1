...
from dynamic_tests.dynamic_tests_predicates_config import ranking_qtype_predicates, survey_dict
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict["ranking_qtype"]
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestAdvBranchVerifyQTypeRankingPredicates/",
                               # report_relative_location
                               "test_advBranch_verifyQTypeRankingPredicates",
                               # report_file_name_prefix
                               ("Verify setting condition selecting all the predicates in condition menu and select "
                                "diff actions for each predicate for Ranking question type "),
                               # test_suite_title
...
@pytest.mark.IB
@pytest.mark.C281053
@pytest.mark.parametrize("test_info", ranking_qtype_predicates,
                         ids=[dict["predicate_label"] for dict in ranking_qtype_predicates])
def test_advBranch_verifyQTypeRankingPredicates(create_survey, test_info):
    """
     Description :
