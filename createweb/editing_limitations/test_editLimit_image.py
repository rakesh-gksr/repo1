...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestEditLimitImage/",  # report_relative_location
                               "test_editLimit_image",  # report_file_name_prefix
                               "verify editing image on image question type after collecting responses",  # test_suite_title
                               ("Adds an image type question. Survey answers are collected"
                                " and verifies that limited editability is enabled, and url can be changed."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_editLimit_image(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify editing image on image question type after collecting responses.")
    try:
        ...
        ex = mySurvey.myQuestion.enter_image_url(
            "https://resources.monkeytest1.com/smassets/createweb/smlib.globaltemplates/1.4.4/assets/logo/surveymonkey_logo_dark.svg")
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Entered New image URL successfully",
                                 "verifies that new url entered successfully",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify image url change."
        mySurvey.myQuestion.click_question_save_from_edit_tab()
