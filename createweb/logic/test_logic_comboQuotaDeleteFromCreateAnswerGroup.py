...
Test case for verifying deletion of quota by clicking back button from  create answer group
"""
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "test_logic_comboQuota_delete_from_create_answer_group",  # report_file_name_prefix
                               "verify deletion of combo quota from create answer group step",  # test_suite_title
                               ("This test deletion of combo quota"
                                " from the create answer group step"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.C83908
@pytest.mark.IB
def test_logic_comboQuotaDeleteFromCreateAnswerGroup(create_survey):
    """
    Test Case Name : combination quota: verify deleting combination quota clicking on back button on create answer groups field
    :param create_survey:
    :return:
    """
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify deletion of combo quota from create answer group step.")
    try:
        ...
        mySurvey.myLogic.attempt_comboQuotaSetupWizard_deleteQuota("combo", [1, 2], [1, 4], [1, 1])
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify Combo quota back navigation",
                                 "checks to make sure that Quota is not added by checking quota status.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
