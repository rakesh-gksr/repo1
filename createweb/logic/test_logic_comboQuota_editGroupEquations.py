...
@pytest.mark.C83895
@pytest.mark.IB
def test_logic_comboQuota_delete_and_edit_group_equation(create_survey):
    ...
    try:
        ...
        mySurvey.myLogic.click_comboQuota_EditEquationGroup([1, 2], [2, 2])

        ex = mySurvey.myLogic.check_comboQuota_GroupExist("Group 2 - Group 2")
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that group edit done",
                                 "checks to make sure that edit group equation is working.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify presence of edited group equation"
