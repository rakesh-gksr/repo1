...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicDisqualify/",  # report_relative_location
                               "test_logic_disqualify",  # report_file_name_prefix
                               "verify disqualifying page",  # test_suite_title
                               ("This test adds 2 pages with 1 question each. Test "
                                "adds page skip logic from P1 -> DQ "
                                " and then checks to make sure that DQ page is "
                                "presented."),  # test_suite_description
                               "utf-8",  # file_encoding
                               {SplunkDataAttributes.TYPE: \
                                   SplunkTestCaseTypes.REGRESSION})  # selenium_driver
    import datetime
    import os
...
def test_logic_disqualify(create_survey):
    ...
    try:
        ...
        mySurvey.myQuestion.enter_question_title(
        ...
        report.add_report_record(
            ...
            "Add page skip logic to P1",
            "Adds page skip logic to P1 to skip to DQ.",
            ex,
            True,
            not ex,
            driver)
        assert ex, "Failed to add page skip logic from P1 to DQ"
        # time.sleep(10)
        ...
        report.add_report_record(
            ReportMessageTypes.TEST_STEP,
            "Check for DQ page",
            "Check to make sure we don't hit page 2 and get taken to DQ page.",
            myBool,
            True,
            not myBool,
            driver)
        assert myBool, "Failed to verify DQ page"
    except:
        report.add_report_record(
            ReportMessageTypes.TEST_STEP,
