...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicEditBlock/",  # report_relative_location
                               "test_logic_editBlock",  # report_file_name_prefix
                               "verify editing block",  # test_suite_title
                               ("This test adds pages 1-3 to a Block "
                                " and then verifies block details and breaking blocks apart"),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_editBlock(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify editing block.")
    try:
        # page1
        ...
        ex = mySurvey.myLogic.click_blockRandomization_getDetails([1, 2, 3])
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify block details",
                                 "checks block details to verify it contains all pages.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify block details."
        # time.sleep(1)
        ex = mySurvey.myLogic.click_blockRandomization_killBlock([1, 2, 3])
