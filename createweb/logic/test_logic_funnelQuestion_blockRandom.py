...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionBlockRandom/",  # report_relative_location
                               "test_logic_funnelQuestion_blockRandom",  # report_file_name_prefix
                               "Test Sender and Receiver can be part of the same block if Sender is before the receiver",  # test_suite_title
                               ("This test adds a sender and receiver question and adds them to the same block."
                                " Test then attempts to funnel accordingly."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_blockRandom(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test Sender and Receiver can be part of the same block if Sender is before the receiver.")
    try:
