...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionMultiReceive/",  # report_relative_location
                               "test_logic_funnelQuestion_multiReceive",  # report_file_name_prefix
                               "Test delete a sender question which has multiple receiver",  # test_suite_title
                               ("This test adds 3 questions(1 sender and 2 receiver) "
                                " and then funnels questions the questions. Test then deletes"
                                " sender which deletes all questions its attached to."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_multiReceive(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test delete a sender question which has multiple receiver.")
    try:
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify questions nuked",
                                 "checks to make sure that all questions were deleted due to deleting sender question",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
