...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionNewQuestionRow/",  # report_relative_location
                               "test_logic_funnelQuestion_newQuestionRow",  # report_file_name_prefix
                               "Test adding new answer choices after the funneled choices",  # test_suite_title
                               ("This test adds a sender and receiver question and funnels accordingly."
                                " Test then attempts to add an answer row after the funneled choices."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_newQuestionRow(create_survey):
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test adding new answer choices after the funneled choices.")
    try:
        ...
        mySurvey.myQuestion.enter_multipleChoice_answerText(4, "Bismarck")
        ex = mySurvey.myQuestion.verifyFunneledRowsLast()
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify last row is a Funneled Row",
                                 "checks to make sure that the last row is a funneled row",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
