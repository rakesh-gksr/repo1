...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionReceiverDelete/",  # report_relative_location
                               "test_logic_funnelQuestion_receiverDelete",  # report_file_name_prefix
                               "Test delete a receiver question",  # test_suite_title
                               ("Test adds a sender and receiver question and forwards accordingly"
                                " Test then deletes receiver question and checks to make sure"
                                " sender question icon is removed."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_receiverDelete(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test delete a receiver question.")
    try:
