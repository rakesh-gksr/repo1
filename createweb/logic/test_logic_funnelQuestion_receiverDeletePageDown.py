...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionReceiverDeletePageDown/",  # report_relative_location
                               "test_logic_funnelQuestion_receiverDeletePageDown",  # report_file_name_prefix
                               "Test delete a receiver question page and move receiver question down",  # test_suite_title
                               ("Test adds a sender and receiver question and forwards accordingly"
                                " Test then deletes the page containging the receiver question,"" moves the receiver question down and"
                                " checks to make sure sender question icon is still active."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_receiverDeletePageDown(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test delete a receiver question page and move receiver question down.")
    try:
        ...
        # time.sleep(3)
        mySurvey.myCreate.nuke_page_move_questions(2)
        # time.sleep(1)
        # driver.refresh() #temporary work around for CREATE-4487, remove after fix
        ex = mySurvey.myLogic.verifyFunnelSenderIcon()
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify icon remains",
                                 "checks to make sure that the icon next to the sender question is still present",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify icon."
    except:
