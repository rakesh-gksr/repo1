...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionReceiverRandom/",  # report_relative_location
                               "test_logic_funnelQuestion_receiverRandom",  # report_file_name_prefix
                               "Test randomize a receiver question",  # test_suite_title
                               ("This test adds a sender and receiver question and funnels accordingly."
                                " Test then attempts to randomize the receiver page."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_receiverRandom(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test randomize receiver page.")
    try:
        ...
        ex = mySurvey.myLogic.verifyPageRandomizationExistingBlock([2])
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify disabled page randomization",
                                 "checks to make sure that all page randomization disabled",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify disabled page randomization."
    except:
