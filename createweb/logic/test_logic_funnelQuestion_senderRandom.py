...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicFunnelQuestionSenderRandom/",  # report_relative_location
                               "test_logic_funnelQuestion_senderRandom",  # report_file_name_prefix
                               "Test randomize a sender question",  # test_suite_title
                               ("This test adds a sender and receiver question and funnels accordingly."
                                " Test then attempts to randomize the sender page."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_funnelQuestion_senderRandom(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test randomize sender page.")
    try:
        ...
        ex = mySurvey.myLogic.verifyPageRandomizationExistingBlock([1])
