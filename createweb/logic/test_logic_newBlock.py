...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicNewBlock/",  # report_relative_location
                               "test_logic_newBlock",  # report_file_name_prefix
                               "verify creating block",  # test_suite_title
                               ("This test adds pages 1-3 to a Block "
                                " and then verifies block creation"),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_newBlock(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify creating block.")
    try:
        # page1
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify block creation",
                                 "checks to make sure that block creation.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
