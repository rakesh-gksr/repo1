...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicPageFlipSelectedWithPageLogic",  # report_relative_location
                               "test_logic_pageFlip_selected_with_page_logic",  # report_file_name_prefix
                               "To verify page skip logic is disbaled when all survey pages has flip randomisation"
                               "for selected pages",  # test_suite_title
                               ("This test applies verifies that page skip logic "
                                " can not be applied when selected Survey pages are Fliped."),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.IB
@pytest.mark.C212997
def test_logic_page_flip_selected_with_page_logic(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify page skip logic is disbaled when "
                                                           "all survey pages has flip randomisation for selected pages")
    try:
        ...
        expected_option_list = [u'-- Choose Page --', u'P3', u'End of Survey', u'Disqualify Respondent']
