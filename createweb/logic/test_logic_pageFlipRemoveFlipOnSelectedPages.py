...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "verify remove flip on selected pages",  # test_suite_title
                               ("This test verifies remove page flip on selected pages"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.IB
@pytest.mark.C212979
def test_logic_pageRemoveFlipOnSelectedPages(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify flip page logic on selected pages.")
    try:
        ...
        ex = mySurvey.myLogic.verifyPageRandomizationStatus('On')
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify Page Randomization status.",
                                 "checks to verify Randomization status is on.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
