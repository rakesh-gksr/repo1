...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "verify removal of flip all pages randomization",  # test_suite_title
                               ("This test verifies to remove page flip "
                                "randomization with all pages selected"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.IB
@pytest.mark.C212978
def test_logic_pageFlipRemoveRandomizationOnAllPages(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify remove flip page radomization"
                                                           " with all pages selected")
    try:
