...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "verify all randomized pages aren't showing in block logic",  # test_suite_title
                               ("This test verifies all randomized"
...
def test_logic_pageRandomSelectedRandomizedPageDisabledInBlockLogic(create_survey):
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify all randomized pages aren't showing in block logic")
    try:
        ...
        ex = mySurvey.myLogic.verifyBlockRandomoizationOnRandomizedPages([1, 2, 3])
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that all the randomized page are disabled",
                                 "checks to make sure that page skip logic cannot be applied to any pages "
                                 "because they are already randomized",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
