...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               ("This test verifies notification when page has skip logic"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.C212952
@pytest.mark.IB
def test_logic_pageRandomAllPagesVerifyNotificationWhenOnePageHasPageSkipLogic(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify notification on page randomization "
                                                           "while randomizing all pages and one of them "
                                                           "is page skip logic destination page")
    try:
        pagewise_questions = [
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that error notification is shown when pages are not selected",
