...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "test_logic_pageRandomSelectedPagesPostPageMove",  # report_file_name_prefix
...
@pytest.mark.TC_13
def test_logic_pageRandomSelectedPagesPostPageMove(create_survey):
    ...
    try:
        ...
        mySurvey.myLogic.click_PageRandomization()
        ex = mySurvey.myLogic.pageRandom_randomizePages_verifySelected([2, 4, 5], len(pagewise_questions))
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that page randomization selected pages post move",
                                 "checks to make sure that post page move, selected page randomization list is proper.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify correct page list shown for page randomization option post move."
