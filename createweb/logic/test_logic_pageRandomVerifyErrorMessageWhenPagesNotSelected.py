...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "verify error notification when pages are not selected",  # test_suite_title
                               ("This test verifies error notification when pages are not selected"),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_pageRandomClickCancelAfterEditingSelectedPages(create_survey):
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify error notification when pages are not selected.")
    try:
        pagewise_questions = [
        ...
        mySurvey.myLogic.pageRandom_randomizePages_selected([])
        ex = mySurvey.myLogic.pageRandomVerifyErrorNotification('You must choose pages to randomize.')
