...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicPageRotate/",  # report_relative_location
                               "test_logic_pageRotate",  # report_file_name_prefix
...
def test_logic_pageRotate(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify applying page rotate page logic.")
    try:
        # page1
        ...
        for x in xrange(page_counts):
            ...
            if x == 0:
                mySurvey.myLogic.PageRotateStartingPage(page_counts)
            # time.sleep(1)
