...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "To verify page rotation after applying logic and deleting selected page.",  # test_suite_title
                               ("This test verifies remove page rotation with selected pages"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.IB
@pytest.mark.C212990
def test_logic_pageRotateOnSelectedPagesDeleteSelectedPage(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "To verify page rotation after "
    ...
    try:
        ...
        # pytest.set_trace()
        mySurvey.myLogic.pageRandom_rotatePages_selected([2, 4, 5])
        ex = mySurvey.myLogic.checkPagesRandomizedIconSelected()
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that page randomization icon appears on 3 pages",
        ...
        mySurvey.myLogic.click_PageRandomization()
        ex = mySurvey.myLogic.pageRandom_rotatePages_verifySelected([2, 4], 4)
        report.add_report_record(ReportMessageTypes.TEST_STEP,
                                 "Verify that page rotation selected pages after deleting selected page",
                                 "checks to make sure that post deleting selected page, "
                                 "selected page rotation list is proper.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify correct page list shown for page rotation option post deleting selected page."

        ex = mySurvey.myLogic.checkPagesRandomizedIconSelected(2)
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that page randomization icon appears on 2 pages",
                                 "checks to make sure that the selected page randomization icon appears.",
                                 ex,
                                 True,
                                 not ex,
