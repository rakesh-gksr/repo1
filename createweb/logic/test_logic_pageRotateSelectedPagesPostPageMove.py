...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "test_logic_pageRotateSelectedPagesPostPageMove",  # report_file_name_prefix
                               "verify page rotation with selected pages after moving pages",  # test_suite_title
                               ("This test applies page logic "
                                " page rotation and checks if correct "
                                " pages are shown post page move."),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.TC_14
def test_logic_pageRotateSelectedPagesPostPageMove(create_survey):
    ...
    try:
        ...
        mySurvey.myLogic.click_PageRandomization()
        ex = mySurvey.myLogic.pageRandom_rotatePages_verifySelected([2, 4, 5], len(pagewise_questions))
        report.add_report_record(ReportMessageTypes.TEST_STEP,
                                 "Verify that page rotation for selected pages post move works",
                                 "checks to make sure that post page move, selected page rotation list is proper.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify correct page list shown for page rotation option post page move."
