...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicPageRotateWithPageLogic/",  # report_relative_location
                               "test_logic_page_rotate_with_page_logic",  # report_file_name_prefix
                               "verify rotated pages are not available for page skip logic ",  # test_suite_title
                               ("This test cases verifies that pages on which page rotation is applied"
                                " are not available for page skip logic."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_page_rotate_with_page_logic(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify that page skip logic can be applied on "
                                                           " pages which don't have page randomisation set")
    try:
