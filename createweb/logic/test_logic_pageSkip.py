...
def test_logic_verifyClearLogic(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify clearing page skip logic.")
    try:
        mySurvey.myLogic.click_PageSkipLogicButton()
        mySurvey.myLogic.editPageSkipLogic(1)
        mySurvey.myLogic.clearPageSkipLogic()
        # time.sleep(3)
        ex = mySurvey.myLogic.verify_page_skip_logic_applied(0, 1)
        ex = not ex
        report.add_report_record(ReportMessageTypes.TEST_STEP, "remove page skip logic to P1",
                                 "Removes page skip logic to P1 to skip to P2.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to remove page skip logic from P1 to P2"
    except:
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Exception occured during test setup",
                                 traceback.format_exc(),
                                 False,
