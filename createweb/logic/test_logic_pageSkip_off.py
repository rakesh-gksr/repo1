...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicPageSkipOff/",  # report_relative_location
                               "test_logic_pageSkip_off",  # report_file_name_prefix
                               "verify page skip logic turns OFF once added",  # test_suite_title
                               ("This test will test that page skip logic turns OFF."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_pageSkip_off(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify page skip OFF.")
    try:
