...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               ...
                               "verify flipping all question with multiple flips",  # test_suite_title
                               ("This test applies page logic to verify working of multiple flips"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.C83927
def test_logic_questionFlipAllQuestionWithMultipleFlips(create_survey):
    ...
    try:
        ...
        mySurvey.myLogic.questionRandom_flipQuestions(2)
        ex = mySurvey.myLogic.checkQuestionsRandomizedIcon()
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify that Question randomization icon appears",
                                 "checks to make sure that the Question randomization icon appears.",
                                 ex,
            ...
            if flag == 0:
                for x in xrange(5):
                    ...
                    if not ex:
                        ...
                        flag = 1
                        report.add_report_record(ReportMessageTypes.TEST_STEP, "Successfully Verified Page 1 Question " + str(x + 1),
        ...
        if attemptCounter < 20 and previewBool:
            myBool = True
        else:
            myBool = False
