...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                            ...
                            "test_logic_questionRandomizationOnQuestionsMoveAllQuestions",  # report_file_name_prefix
                            ("verify question randomization menu update while moving a "
                            " question to a page with all questions randomized"),  # test_suite_title
                            ("This test applies page logic "
                            " question randomization on all questions and then question is moved "
                            " and its verified if its working as expected"),  # test_suite_description
                            "utf-8",  # file_encoding
...
@pytest.mark.C284164
@pytest.mark.IB
def test_logic_questionRandomizationOnQuestionsMoveAllQuestions(create_survey):
    ...
    try:
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP,
                                 "Verify that Question randomization menu has question which is moved",
                                 "checks to make sure that the question appears in randomization menu post move.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
