...
def test_logic_questionSkip_moveDestination(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, ("verify adding question skip logic having "
                                                            "destination question on the same page above the source question."))
    try:
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify question skip logic applied",
                                 "Verifies question skip logic icon appears.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify question skip logic applied."
        mySurvey.myQuestion.click_on_question_to_edit(3)
        mySurvey.myQuestion.click_question_move_tab()
        mySurvey.myLogic.moveQuestion(3, 5)
        mySurvey.myQuestion.click_on_question_to_edit(2)
        mySurvey.myQuestion.click_question_logic_tab()
        ...
        mySurvey.myQuestion.click_on_question_to_edit(5)
        mySurvey.myQuestion.click_question_move_tab()
        mySurvey.myLogic.moveQuestion(1, 1)
        mySurvey.myQuestion.click_on_question_to_edit(3)
        mySurvey.myQuestion.click_question_logic_tab()
        ex = mySurvey.myLogic.verify_questionSkipLogic(2, 1, 2)
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify question skip logic update",
                                 "Verifies that question skip logic updates with new destination coordinates.",
                                 ex,
                                 True,
        ...
        mySurvey.myQuestion.click_question_move_tab()
        mySurvey.myLogic.moveQuestion(2, 4)
