...
def test_logic_questionSkip_skipPageDown(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(
        ReportMessageTypes.TEST_CASE,
        "verify adding question skip logic having destination question on the same page above the source question with skipPageDown")
    try:
        ...
        mySurvey.myLogic.setQuestionSkipLogic(2, 2, False, 2)
        mySurvey.myQuestion.click_on_question_to_edit(1)
        mySurvey.myQuestion.click_question_logic_tab()
        ex = mySurvey.myLogic.verify_questionSkipLogic(2, 2, 2)
