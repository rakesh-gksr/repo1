...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestLogicRemovingRandomizationLogicOnSelectedPages.py/",  # report_relative_location
                               "test_logic_removingRandomizationLogicOnSelectedPages",  # report_file_name_prefix
                               "verify removing randomization logic on selected pages",  # test_suite_title
                               ("This test applies randomization logic "
                                " on selected pages. "
                                " to remove randomization logic on selected pages"),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_logic_removingQuestionRandomizationOnAllQuestionsOnSelectedPages(create_survey):
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify removing randomization logic on selected pages.")
    try:
        # Question 1
        ...
        print ex
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify removing randomization logic on selected pages",
