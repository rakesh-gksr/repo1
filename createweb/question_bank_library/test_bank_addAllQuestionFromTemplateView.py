...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestBankAddAllQuestionFromTemplateView/",  # report_relative_location
                               "test_bank_addAllQuestionFromTemplateView",  # report_file_name_prefix
                               "Verify Add all Questions from Template View",  # test_suite_title
                               ("Test to verify Add all Questions from Template View"),  # test_suite_description
                               "utf-8",  # file_encoding
...
@pytest.mark.IB
@pytest.mark.QBL
def test_bank_addAllQuestionFromTemplateView(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify Add all Questions from Template View")
    ...
    try:
        ...
        totalQuestions = int(mySurvey.myQBank.getTotalQuestions())
        ex = mySurvey.myQBank.verifyTotalQuestionAddedInPreview(totalQuestions) # QBL-200
