...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestBankVerifyPreviewWarningBasicUserWithCollector/",  # report_relative_location
                               "test_bank_verifyPreviewWarningBasicUserWithCollector",  # report_file_name_prefix
                               "Verify Basic User with a Collector tries to "
                               "add more than 10 questions via the modal",  # test_suite_title
                               ("Test to verify Basic User with a Collector tries to "
...
@pytest.mark.IB
@pytest.mark.QBL
def test_bank_verifyPreviewWarningBasicUserWithCollector(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "Basic User with a Collector tries to "
    ...
    try:
        ...
        mySurvey.mySvc_holder.collectsvc.create(int(mySurvey.user_id), params)
        ...
        ex = mySurvey.myQBank.verifyUpgradeInPreviewQuestionBox("Disabled")
        ...
        ex = mySurvey.myQBank.verifyTotalQuestionAddedInPreview(10)
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify not able to add more than 10 questions",
                                 "check to make sure that not added more than 10 questions in preview",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "More than 10 questions added in preview"
