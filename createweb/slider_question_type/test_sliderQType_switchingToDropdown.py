...
def get_report(request):
    ...
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestSQTSwitchingToDropdown/",  # report_relative_location
                               "test_sliderQType_switchingToDropdown",  # report_file_name_prefix
                               "verify switching from slider to Dropdown question type, saving them and back "
                               "to slider can be saved without an error showing default settings",  # test_suite_title
                               ("Test to Verify switching from slider to Dropdown question type, saving "
...
@pytest.mark.BVT
@pytest.mark.C410831
def test_sliderQType_switchingToDropdown(create_survey):
    driver, mySurvey, report = create_survey
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Test to Verify switching from slider to Dropdown "
    ...
    try:
        ...
        # code to change slider question type to dropdown textbox
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP,
                                 "Verify change question type from Slider to Dropdown",
                                 "verifies that changed question type from Slider to Dropdown",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        ...
        mySurvey.myQuestion.click_on_question_to_edit()
        # code to change dropdown textbox question type to slider
