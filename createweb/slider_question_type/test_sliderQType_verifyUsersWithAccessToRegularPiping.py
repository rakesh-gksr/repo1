...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestSliderQTypeVerifyUsersWithAccessToRegularPiping/",  # report_relative_location
                               "test_sliderQType_verifyUsersWithAccessToRegularPiping",  # report_file_name_prefix
                               "Verify users with access to regular piping should also be able to pipe in the "
                               "respondent's answer to a slider question into question text of a followup question.",
                               # test_suite_title
                               ("Test to verify users with access to regular piping should also be able to pipe in the "
                                "respondent's answer to a slider question into question text of a followup question."),
...
@pytest.mark.IB
@pytest.mark.C284119
def test_sliderQType_verifyUsersWithAccessToRegularPiping(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "Verify users with access to regular piping should also be "
                                                           "able to pipe in the respondent's answer to a slider "
                                                           "question into question text of a followup question.")
    try:
