...
# -*- coding: utf-8 -*-
"""
verify moving slider with position to the center.
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestSliderQuestionVerifyMoveSliderWithPositionCenter/",  # report_relative_location
                               "test_sliderQuestionVerifyMoveSliderWithPositionCenter",  # report_file_name_prefix
                               # test_suite_title
...
@pytest.mark.IB
@pytest.mark.C286973
def test_sliderQuestionVerifyMoveSliderWithPositionCenter(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify moving slider with position to the center")
    try:
        ...
        ex = mySurvey.myQuestion.selectSliderStartPosition('center')
        report.add_report_record(ReportMessageTypes.TEST_STEP,
                                 "Verify that slider question center position selected",
                                 "checks to make sure that slider question center is selected.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify slider question center is selected"
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP,
                                 "verify moving slider question with position to the center",
