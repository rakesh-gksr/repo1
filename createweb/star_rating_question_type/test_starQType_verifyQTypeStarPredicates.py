...
from dynamic_tests.dynamic_tests_config import star_qtype_predicates, survey_dict
import traceback
import pytest
...
def pre_setup_survey(mySurvey, driver, report):
    ...
    survey_info = survey_dict[pytest.config.option.domain[:3].upper()]["star_qtype_adv_branch"]
...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestStarQTypeVerifyQTypeStarPredicates/",
                               # report_relative_location
                               "test_starQType_verifyQTypeStarPredicates",
                               # report_file_name_prefix
                               "Verify hiding and showing star question on branching action condition",
                               # test_suite_title
                               ("This test suite adds show and hide star question predicates in condition menu "
...
@pytest.mark.C812401
@pytest.mark.C921898
@pytest.mark.parametrize("test_info", star_qtype_predicates,
                         ids=[dict["title"] for dict in star_qtype_predicates])
def test_starQType_verifyQTypeStarPredicates(create_survey, test_info):
    """
        Description :
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, dict["title"])
    try:
        ...
        report.add_report_record(ReportMessageTypes.TEST_STEP, "Verify Advanced Branching - " + test_info["title"],
                                 "verifies current advanced branching ruleset.",
                                 ex,
                                 True,
                                 not ex,
                                 driver)
        assert ex, "Failed to verify advanced branching - " + test_info["title"]
