...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestStarQTypeVerifyUsersWithAccessToRegularPiping/",  # report_relative_location
                               "test_starQType_verifyUsersWithAccessToRegularPiping",  # report_file_name_prefix
                               "verify users with access to regular piping should also be able to pipe in the "
                               "respondent's answer to a star question into question text of a followup question..",
                               # test_suite_title
...
@pytest.mark.BVT
@pytest.mark.C812376
def test_starQType_verifyUsersWithAccessToRegularPiping(create_survey):
    ...
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify users with access to regular piping should also be "
                                                           "able to pipe in the respondent's answer to a star question "
                                                           "into question text of a followup question.")
    try:
