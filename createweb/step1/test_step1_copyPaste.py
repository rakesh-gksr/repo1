...
def get_report(request):
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestStep1CopyPaste/",  # report_relative_location
                               "test_step1_copyPaste",  # report_file_name_prefix
                               "Test creating new survey via step1 copyPaste",  # test_suite_title
                               ("Test to make sure step 1 survey creation functions with copyPaste "),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_step1_copyPaste(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "test_step1_copyPaste")
    try:
...
def test_step1_copyPaste_switchMutlipleChoice(create_survey):
    driver, mySurvey, report = create_survey
...
def test_step1_copyPaste_singleTextbox(create_survey):
    driver, mySurvey, report = create_survey
...
@pytest.mark.skipif(True, reason="skip this test as new copy & paste questions window do not have the learn more "
                               "and skip page links")
def test_step1_copyPaste_skip(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "test_step1_copyPaste_skip")
    try:
        mySurvey.myCreate.click_new_survey()
        step1 = NewSurvey(driver)
        step1.create_survey_from_scratch()
...
def test_step1_copyPaste_largeLoad(create_survey):
    driver, mySurvey, report = create_survey

    report.add_report_record(ReportMessageTypes.TEST_CASE, "test_step1_copyPaste_largeLoad")
    try:
        mySurvey.myCreate.click_new_survey()
        step1 = NewSurvey(driver)
        step1.create_survey_from_scratch()
