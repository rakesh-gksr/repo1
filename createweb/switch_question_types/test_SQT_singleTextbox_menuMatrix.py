...
def get_report(request):
    ...
    report = reporting_wrapper("Create",  # report_feature_name
                               "TestSQTSingleTextboxMenuMatrix/",  # report_relative_location
                               "test_SQT_singleTextbox_menuMatrix",  # report_file_name_prefix
                               "verify switching from single textbox to menumatrix ",  # test_suite_title
                               ("This test suite adds a single textbox question and switches it to a menu matrix question   "
                                " Test verifies question saves."),  # test_suite_description
                               "utf-8",  # file_encoding
...
def test_SQT_singleTextbox_menuMatrix(create_survey):
    driver, mySurvey, report = create_survey

    # then start the normal test
    report.add_report_record(ReportMessageTypes.TEST_CASE, "verify switching from single textbox to menumatrix")
    try:
